...
import { WebhookSignatureVerifier, VerificationConfig } from '../../../src/lib/webhook/signature-verification.js';
...
describe('WebhookSignatureVerifier', () => {
  ...
  describe('verifyTwilioSignature', () => {
    it('should verify valid Twilio signature', () => {
      ...
      const config: VerificationConfig = { secret: testSecret };
      const result = WebhookSignatureVerifier.verifyTwilioSignature(
        testUrl,
        testPayload,
        expectedSignature,
        config
  ...
  describe('verifyWhatsAppSignature', () => {
    it('should verify valid WhatsApp signature', () => {
      ...
      const result = WebhookSignatureVerifier.verifyWhatsAppSignature(
        testPayload,
  ...
  describe('verifySlackSignature', () => {
    it('should verify valid Slack signature', () => {
      ...
      const config: VerificationConfig = { secret: testSecret, tolerance: 300 };
      const headers = {
        'x-slack-signature': `v0=${expectedSignature}`,
        'x-slack-request-timestamp': timestamp.toString()
      };

      const result = WebhookSignatureVerifier.verifySlackSignature(
        testPayload,
        headers,
        config
      );
  ...
  describe('verifyGoogleSignature', () => {
    it('should verify valid Google token', () => {
      const config: VerificationConfig = { secret: 'google-channel-token' };
      const headers = {
        'x-goog-channel-token': 'google-channel-token'
      };

      const result = WebhookSignatureVerifier.verifyGoogleSignature(
        testPayload,
        headers,
        config
      );
    ...
    it('should reject invalid Google token', () => {
      const config: VerificationConfig = { secret: 'correct-token' };
      const headers = {
  ...
  describe('verifyHMACSignature', () => {
    it('should verify SHA256 HMAC signature', () => {
      ...
      const result = WebhookSignatureVerifier.verifyHMACSignature(
        testPayload,
        expectedSignature,
        config,
        'test-provider'
  ...
  describe('verifyWebhookSignature', () => {
    it('should route Twilio verification correctly', () => {
      ...
      const result = WebhookSignatureVerifier.verifyWebhookSignature(
        'twilio',
        testPayload,
        headers,
        config,
  ...
  describe('generateSignature', () => {
    it('should generate Twilio signature', () => {
      const signature = WebhookSignatureVerifier.generateSignature(
        'twilio',
        testPayload,
        testSecret,
        { url: testUrl }
    ...
    it('should throw error when URL required but not provided', () => {
      expect(() => {
        WebhookSignatureVerifier.generateSignature(
          'twilio',
          testPayload,
          testSecret
          // No URL provided
