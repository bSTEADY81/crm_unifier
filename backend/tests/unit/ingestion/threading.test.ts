...
import { NormalizedMessage } from '../../../src/lib/ingestion/types.js';
...
describe('ConversationGrouper', () => {
  ...
  describe('groupIntoConversation', () => {
    it('should find existing conversation by thread key', async () => {
      ...
      const result = await ConversationGrouper.groupIntoConversation(
        normalizedMessage,
        testCustomer1.id
      );
  ...
  describe('generateThreadKey', () => {
    it('should generate consistent thread keys for SMS', () => {
      const key1 = ConversationGrouper.generateThreadKey(
        'sms',
        '+1234567890',
        '+0987654321'
      );
      const key2 = ConversationGrouper.generateThreadKey(
        'sms',
        '+0987654321', // Reversed order
        '+1234567890'
      );
    ...
    it('should generate thread keys for WhatsApp', () => {
      const key = ConversationGrouper.generateThreadKey(
        'whatsapp',
        '+1234567890',
        '+0987654321'
      );
    ...
    it('should use additional context for email thread keys', () => {
      const keyWithContext = ConversationGrouper.generateThreadKey(
        'email',
        'user@example.com',
        'support@company.com',
        'gmail-thread-123'
      ...
      const keyWithoutContext = ConversationGrouper.generateThreadKey(
        'email',
        'user@example.com',
        'support@company.com'
      );
  ...
  describe('updateConversationActivity', () => {
    ...
    it('should add new tags to conversation', async () => {
      ...
      expect(updatedConversation?.tags).toEqual(
        expect.arrayContaining([...originalTags, ...newTags])
      );
    });
  ...
  describe('generateContextualThreadKey', () => {
    it('should add subject context for emails', () => {
      ...
      const contextualKey = ConversationGrouper.generateContextualThreadKey(
        baseKey,
    ...
    it('should normalize email subjects consistently', () => {
      ...
      const key1 = ConversationGrouper.generateContextualThreadKey(
        baseKey,
        { subject: 'Support Request' }
      );
      
      const key2 = ConversationGrouper.generateContextualThreadKey(
        baseKey,
        { subject: 'Re: Support Request' }
      );
      
      const key3 = ConversationGrouper.generateContextualThreadKey(
        baseKey,
