...
describe('MessageDeduplicator', () => {
    ...
    describe('checkProviderIdDuplicate', () => {
        it('should detect duplicate provider message IDs', async () => {
            ...
            const result = await MessageDeduplicator.checkProviderIdDuplicate(testProviderId, 'SM-duplicate-test-001');
        ...
        it('should not detect duplicate for unique provider message ID', async () => {
            const result = await MessageDeduplicator.checkProviderIdDuplicate(testProviderId, 'SM-unique-message-001');
    ...
    describe('checkForDuplicate', () => {
        it('should perform comprehensive duplicate check', async () => {
            ...
            const result = await MessageDeduplicator.checkForDuplicate(normalizedMessage, rawMessage);
            expect(result.isDuplicate).toBe(true);
            expect(result.duplicateType).toBe('provider_id');
        });
        it('should detect content hash duplicates', async () => {
            ...
            const result = await MessageDeduplicator.checkForDuplicate(normalizedMessage, rawMessage, { checkContentDuplicates: true, timeWindowMinutes: 60 });
            expect(result.isDuplicate).toBe(true);
...
describe('IdempotencyManager', () => {
    describe('generateIdempotencyKey', () => {
        it('should generate consistent keys for same input', () => {
            ...
            const key1 = IdempotencyManager.generateIdempotencyKey(providerId, providerMessageId, timestamp);
            const key2 = IdempotencyManager.generateIdempotencyKey(providerId, providerMessageId, timestamp);
        ...
        it('should generate different keys for different inputs', () => {
            const providerId = 'test-provider-001';
            const timestamp = new Date('2023-10-01T10:00:00Z');
            const key1 = IdempotencyManager.generateIdempotencyKey(providerId, 'message-001', timestamp);
            const key2 = IdempotencyManager.generateIdempotencyKey(providerId, 'message-002', timestamp);
